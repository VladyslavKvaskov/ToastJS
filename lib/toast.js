...
class Toaster {
  constructor(config) {
    ...
    toastMessage.innerHTML = `
        ...
        ${toastCoords[this.position] ? '<div class="toast-actions"><button type="button" class="ripple reset-coords act-bttn">reset coords</button></div>' : ''}
    `;
    ...
    if (this.draggable) {
      ...
      const moveToast = (e) => {
        if (canDragToast) {
          ...
          const toastX = (pageXY.x / window.innerWidth * 100) - (toastShiftX / window.innerWidth * 100);
          const toastY = (pageXY.y / window.innerHeight * 100) - (toastShiftY / window.innerHeight * 100);
          ...
          if (!toastMessage.querySelector('.reset-coords')) {
            toastMessage.insertAdjacentHTML('beforeend', `<div class="toast-actions"><button type="button" class="ripple reset-coords act-bttn">reset coords</button></div>`);
            resetToast();
          }
        }
      }
...
const cssTxt = `
  ...
  toast-message .toast-actions button{
    font-size: var(--toast-font-size-sm);
    border-bottom: 0.1vmax solid var(--toast-info) !important;
  }

  toast-message[data-status="default"] .toast-actions button{
    background-color: var(--toast-default);
    background-image: var(--toast-default-gradient);
  }

  toast-message[data-status="info"] .toast-actions button{
    background-color: var(--toast-info);
    background-image: var(--toast-info-gradient);
  }

  toast-message[data-status="warning"] .toast-actions button{
    background-color: var(--toast-warning);
    background-image: var(--toast-warning-gradient);
    color: var(--toast-default);
  }

  toast-message[data-status="error"] .toast-actions button{
    background-color: var(--toast-error);
    background-image: var(--toast-error-gradient);
  }

  toast-message[data-status="success"] .toast-actions button{
    background-color: var(--toast-success);
    background-image: var(--toast-success-gradient);
    color: var(--toast-default);
  }
