...
class Ripple extends HTMLElement {
  ...
  showRipple(e = '') {
    ...
    if (this.dataset.rippleTarget) {
      this.targetRipple = document.querySelector(this.dataset.rippleTarget);
    }

    const rippleEl = document.createElement('span');
    const rippleDiameter = Math.max(this.targetRipple.clientWidth, this.targetRipple.clientHeight);
    const rippleRadius = rippleDiameter / 2;
    const rippleBttnBounds = this.targetRipple.getBoundingClientRect();
    rippleEl.style.width = rippleEl.style.height = `${rippleDiameter}px`;
    if (e?.isTrusted) {
      rippleEl.style.left = `${e.clientX - rippleBttnBounds.x - rippleRadius}px`;
      rippleEl.style.top = `${e.clientY - rippleBttnBounds.y - rippleRadius}px`;
    } else {
      rippleEl.style.left = `${(rippleBttnBounds.width / 2) - rippleRadius}px`;
      rippleEl.style.top = `${(rippleBttnBounds.height / 2) - rippleRadius}px`;
    }
    ...
    this.targetRipple.appendChild(rippleEl);
  }
}
