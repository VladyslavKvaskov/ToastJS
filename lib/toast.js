...
class Toaster {
  constructor(config) {
    ...
    toastMessage.innerHTML = `
        ...
        ${toastCoords[this.position] ? `<div class="toast-actions"><button type="button" is="ripple-button" class="reset-coords act-bttn" data-ripple-target=".${toastMsgIdClass}">reset coords</button></div>` : ''}
    `;
      ...
      const moveToast = (e) => {
        if (canDragToast) {
          ...
          if (!toastMessage.querySelector('.reset-coords')) {
            toastMessage.insertAdjacentHTML('beforeend', `<div class="toast-actions"><button type="button" is="ripple-button" class="ripple reset-coords act-bttn" data-ripple-target=".${toastMsgIdClass}">reset coords</button></div>`);
            resetToast();
          }
...
const toastCssTXT = `
  ...
  toast-message .toast-actions .reset-coords{
    font-size: var(--toast-font-size-sm);
    border-bottom: 0.1vmax solid var(--toast-info) !important;
  }

  toast-message[data-status="default"] .toast-actions .reset-coords{
    background-color: var(--toast-default);
    background-image: var(--toast-default-gradient);
  }

  toast-message[data-status="info"] .toast-actions .reset-coords{
    background-color: var(--toast-info);
    background-image: var(--toast-info-gradient);
  }

  toast-message[data-status="warning"] .toast-actions .reset-coords{
    background-color: var(--toast-warning);
    background-image: var(--toast-warning-gradient);
    color: var(--toast-default);
  }

  toast-message[data-status="error"] .toast-actions .reset-coords{
    background-color: var(--toast-error);
    background-image: var(--toast-error-gradient);
  }

  toast-message[data-status="success"] .toast-actions .reset-coords{
    background-color: var(--toast-success);
    background-image: var(--toast-success-gradient);
    color: var(--toast-default);
  }
