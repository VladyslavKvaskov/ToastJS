...
class Toaster {
  constructor(config) {
    ...
    this.showConfirmBttn = config.showCloseBttn === true ? true : false;
    this.showDenyBttn = config.showCloseBttn === true ? true : false;
    this.showCancelBttn = config.showCloseBttn === true ? true : false;
        ...
        <div class="content">
          ...
          ${(this.showConfirmBttn || this.showDenyBttn || this.showCancelBttn) ? `
            <div class="action-bttns">
              ${this.showConfirmBttn ? `<button type="button" class="confirm-bttn act-bttn">${this.confirmBttnText?this.confirmBttnText:'ok'}</button>` : ''}
              ${this.showDenyBttn ? `<button type="button" class="deny-bttn act-bttn">${this.denyBttnText?this.denyBttnText:'no'}</button>` : ''}
              ${this.showCancelBttn ? `<button type="button" class="cancel-bttn act-bttn">${this.cancelBttnText?this.cancelBttnText:'cancel'}</button>` : ''}
            </div>
            `: ''}
        </div>
...
const cssTxt = `
  ...
  toast-message[data-status="success"]{
    border: 3px solid var(--toast-success);
  }
  toast-message[data-status="error"]{
    border: 3px solid var(--toast-error);
  }
  toast-message[data-status="info"]{
    border: 3px solid var(--toast-info);
  }
  toast-message[data-status="warning"]{
    border: 3px solid var(--toast-warning);
  }
  toast-message[data-status="default"]{
    border: 3px solid var(--toast-default);
  }
  ...
  toast-message[data-status="error"] .close-bttn:hover::before, toast-message[data-status="error"] .close-bttn:hover::after, toast-message[data-status="error"] .close-bttn:focus::before, toast-message[data-status="error"] .close-bttn:focus::after {
    background-color: var(--toast-error);
  }
  toast-message[data-status="error"] .close-bttn:hover, toast-message[data-status="error"] .close-bttn:focus{
    background-color: #fff;
  }
  ...
  toast-message button[type="button"]:hover, toast-message button[type="button"]:focus{
    box-shadow: 0 0 0 0.1vmax #000;
  }
