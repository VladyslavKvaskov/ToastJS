    ...
    if (this.draggable) {
      ...
      toastMessage.addEventListener('mousedown', (e) => {
        ...
        if (toastContent) {
          if (!e.target.closest('.content') && !e.target.closest('.close-bttn') && e.target.closest('toast-message') === toastMessage) {
            ...
            toastShiftX = (e.clientX || e.touches[0].clientX) - toastBounds.left;
            toastShiftY = (e.clientY || e.touches[0].clientY) - toastBounds.top;
      ...
      const moveToast = (e) => {
        if (canDragToast) {
          ...
          const toastBounds = toastMessage.getBoundingClientRect();
          toastMessage.style.transform = 'none';
          const toastY = (e.pageY || e.touches[0].pageY) - toastShiftY;
          const toastX = (e.pageX || e.touches[0].pageX) - toastShiftX;
